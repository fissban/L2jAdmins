 ...
 * @author fissban
 */
public class PcFishingTask implements Runnable
{
	private L2PcInstance fisher;
	private int time;
	private int stop = 0;
	private int goodUse = 0;
	private PcFishAnimationType animation = PcFishAnimationType.NONE;
	private PcFishModeType mode;
	...
	public PcFishingTask(L2PcInstance fisher, PcFishingInstance fish, PcFishLureType lureType)
	{
		this.fisher = fisher;
		...
		mode = Rnd.get(100) >= 80 ? PcFishModeType.FIGHTING : PcFishModeType.RESTING;
	...
	public void changeHp(int hp, int pen)
	{
		...
		animation = PcFishAnimationType.NONE;
	...
	protected void aiTask()
	{
		...
		try
		{
			if (mode == PcFishModeType.FIGHTING)
			{
			...
			if (stop == 0)
			{
				...
				if ((Rnd.get(100) >= 70) && (mode == PcFishModeType.RESTING))
				{
					mode = PcFishModeType.FIGHTING;
				}
			}
			else
			{
		...
		finally
		{
			...
			if (animation != PcFishAnimationType.NONE)
			{
	...
	{
		animation = PcFishAnimationType.PUMPING;
		if (Rnd.get(100) > 90)
		{
	...
	{
		animation = PcFishAnimationType.REELING;
		if (Rnd.get(100) > 90)
		{
			fisher.sendPacket(SystemMessage.FISH_RESISTED_ATTEMPT_TO_BRING_IT_IN);
			goodUse = 0;
		...
		if (mode == PcFishModeType.RESTING)
		{
