...
public class Q373_SupplierOfReagents extends Script
{
	// Variables
	...
	// NPCs
	private static final int WESLEY = 7166;
	private static final int URN = 8149;
	
	// Monsters
	private static final int CRENDION = 813;
	private static final int HALLATE_MAID = 822;
	private static final int HALLATE_GUARDIAN = 1061;
	private static final int PLATINUM_TRIBE_SHAMAN = 828;
	private static final int PLATINUM_GUARDIAN_SHAMAN = 1066;
	private static final int LAVA_WYRM = 1111;
	private static final int HAMES_ORC_SHAMAN = 1115;
	...
	public Q373_SupplierOfReagents()
	{
		...
		registerItems(MIXING_STONE, MIXING_MANUAL);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		// Wesley
		if (event.equalsIgnoreCase("7166-04.htm"))
		{
		...
		else if (event.equalsIgnoreCase("7166-09.htm"))
		{
		...
		{
			if (!st.hasItems(MIXING_STONE))
			{
				htmltext = "8149-04.htm";
			}
		}
		else if (event.startsWith("8149-03-"))
		{
			final int regentId = Integer.parseInt(event.substring(8, 12));
			for (final int[] formula : FORMULAS)
			{
				...
				if (st.getItemsCount(regentId) < formula[0])
				{
					break;
				}
		...
		else if (event.startsWith("8149-06-"))
		{
			final int catalyst = Integer.parseInt(event.substring(8, 12));
			
			// Not enough items, cancel the operation.
			if (!st.hasItems(catalyst))
			{
				return "8149-04.htm";
			}
		...
		else if (event.startsWith("8149-12-"))
		{
			...
			for (final int[] formula : FORMULAS)
			{
				...
				// Not enough regents.
				if (st.getItemsCount(regent) < formula[0])
				{
					break;
				}
				...
				final int tempIndex = Integer.parseInt(event.substring(8, 9));
				for (final int temperature[] : TEMPERATURES)
				{
					...
					if (Rnd.get(100) < temperature[1])
					{
						...
						return "8149-12-" + formula[3] + ".htm";
					}
					
					return "8149-11.htm";
				}
			}
			htmltext = "8149-13.htm";
		}
		return htmltext;
	}
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		{
			case CREATED:
				htmltext = player.getLevel() < 57 ? "7166-01.htm" : "7166-02.htm";
				break;
			
			case STARTED:
				if (npc.getId() == WESLEY)
				{
					htmltext = "7166-05.htm";
				}
				else
				{
