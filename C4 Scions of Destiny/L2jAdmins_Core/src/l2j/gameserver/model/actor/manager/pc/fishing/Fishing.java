...
import l2j.gameserver.model.actor.manager.pc.fishing.task.LookingForFish;
import l2j.gameserver.model.actor.manager.pc.fishing.task.StartCombatForFish;
...
public class Fishing
{
	...
	private FishingHolder fish;
	private StartCombatForFish fishCombat;
	...
	public Fishing(L2PcInstance actor)
	{
		player = actor;
	}
	...
	public StartCombatForFish getCombat()
	{
		return fishCombat;
	}
	
	public void startCombat(FishLureType lureType)
	{
		fishCombat = new StartCombatForFish(player, fish, lureType);
	}
	...
	public void startFishing(int x, int y, int z)
	{
		...
		List<FishingHolder> fishs = FishTable.getInstance().getfish(lvl, type, group);
		if (fishs.isEmpty())
		{
	...
	public void startLookingForFishTask()
	{
		if (!player.isDead() && (taskForFish == null))
		{
			...
			taskForFish = ThreadPoolManager.scheduleAtFixedRate(new LookingForFish(player, fish.getWaitTime(), fish.getFishGuts(), fish.getType(), fish.getLureType()), 10000, checkDelay);
		}
	}
